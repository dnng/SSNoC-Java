...
public class SQL {
	...
	public static final String CREATE_USERS = "create table IF NOT EXISTS "
			...
			+ " salt VARCHAR(512)," + " last_status_code_id BIGINT, last_location_id BIGINT, "
	...
	public static final String FIND_ALL_USERS = "select u.user_id, u.user_name, u.password,"
			+ " u.salt, u.last_status_code_id, ssc.status, ssc.created_at, u.last_location_id, slc.location, u.created_at, u.modified_at " + " from " + SSN_USERS + " u "
	...
	public static final String FIND_ALL_STATUS_CRUMBS = "select ssc.status_crumb_id, ssc.user_id, u.user_name, ssc.status, ssc.location_crumb_id, slc.location, "
			+ " ssc.created_at " + " from " + SSN_STATUS_CRUMBS + " ssc "
			+ " left outer join " + SSN_USERS + " u on ssc.user_id = u.user_id "
			+ " left outer join " + SSN_LOCATION_CRUMBS + " slc on ssc.location_crumb_id = slc.location_crumb_id "
	...
	*/
	...
	public static final String INSERT_CHAT_MESSAGE = "insert into " + SSN_CHAT_MESSAGES
			+ " (sender_id, receiver_id, content, location, created_at) values (?, ?, ?, ?, CURRENT_TIMESTAMP())";
